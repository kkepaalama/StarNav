...
'''
File returns position estimate for image belonging to s23 class
'''
...
from ost_output_papakolea import s23
...
phi = s23.imu_tilt[0] #roll
theta = s23.imu_tilt[1] #pitch
psi = s23.imu_tilt[2] #yaw

#converts star coordinates in GCRS (which is a ECI frame) to ITRS (which is a ECEF frame)
e = main.cel2ecef(s23.time, s23.cel[0], s23.radec, 'gcrs')
e1 = main.cel2ecef(s23.time, s23.cel[1], s23.radec, 'gcrs')
e2 = main.cel2ecef(s23.time, s23.cel[2], s23.radec, 'gcrs')
e3 = main.cel2ecef(s23.time, s23.cel[3], s23.radec, 'gcrs')
e4 = main.cel2ecef(s23.time, s23.cel[4], s23.radec, 'gcrs')
e5 = main.cel2ecef(s23.time, s23.cel[5], s23.radec, 'gcrs')
e6 = main.cel2ecef(s23.time, s23.cel[6], s23.radec, 'gcrs')
e7 = main.cel2ecef(s23.time, s23.cel[7], s23.radec, 'gcrs')
e8 = main.cel2ecef(s23.time, s23.cel[8], s23.radec, 'gcrs')
e9 = main.cel2ecef(s23.time, s23.cel[9], s23.radec, 'gcrs')
e10 = main.cel2ecef(s23.time, s23.cel[10], s23.radec, 'gcrs')
e11 = main.cel2ecef(s23.time, s23.cel[11], s23.radec, 'gcrs')
e12 = main.cel2ecef(s23.time, s23.cel[12], s23.radec, 'gcrs')
e13 = main.cel2ecef(s23.time, s23.cel[13], s23.radec, 'gcrs')
e14 = main.cel2ecef(s23.time, s23.cel[14], s23.radec, 'gcrs')
e15 = main.cel2ecef(s23.time, s23.cel[15], s23.radec, 'gcrs')
e16 = main.cel2ecef(s23.time, s23.cel[16], s23.radec, 'gcrs')
ev = main.cel2ecef(s23.time, s23.cel[13], s23.radec, 'radec2car')
...
i = np.array([-0.98080705, -0.18540823, -0.0603437 ])
i1 = np.array([-0.97131329, -0.22795797, -0.06771701])
i2 = np.array([-0.9325916 , -0.34321286, -0.11170441])
i3 = np.array([-0.94893035, -0.3069925 , -0.07271038])
i4 = np.array([-0.93783506, -0.34523944, -0.03571017])
i5 = np.array([-0.96600475, -0.25738727,  0.02422115])
i6 = np.array([-0.96835517, -0.24619784,  0.04092637])
i7 = np.array([-0.93610606, -0.35144263, -0.0139095 ])
i8 = np.array([-0.93466222, -0.35551807, -0.00367045])
i9 = np.array([-0.94169579, -0.3362352 ,  0.01244364])
i10 = np.array([-0.95923328, -0.27839127,  0.04868128])
i11 = np.array([-0.90759622, -0.41925516, -0.02223096])
i12 = np.array([-0.96518413, -0.24072589,  0.10232623])
i13 = np.array([-0.96129626, -0.25643196,  0.10075777])
i14 = np.array([-0.97689351, -0.11906485,  0.17748968])
i15 = np.array([-0.97495729, -0.13674637,  0.17538183])
i16 = np.array([-0.90079414, -0.43346534,  0.02603271])

v = np.array([-0.96964362, -0.2439571 ,  0.01661878]) #boresight vector of the camera
...
#i_v_br = np.dot(main.Rz(np.radians(-s23.imu_tilt[2])), i_v_br1)
