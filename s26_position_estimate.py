...
'''
File returns position estimate for image belonging to s26 class
'''
...
from ost_output_papakolea import s26
...
phi = s26.imu_tilt[0] #roll
theta = s26.imu_tilt[1] #pitch
psi = s26.imu_tilt[2] #yaw

#converts star coordinates in GCRS (which is a ECI frame) to ITRS (which is a ECEF frame)
e = main.cel2ecef(s26.time, s26.cel[0], s26.radec, 'gcrs')
e1 = main.cel2ecef(s26.time, s26.cel[1], s26.radec, 'gcrs')
e2 = main.cel2ecef(s26.time, s26.cel[2], s26.radec, 'gcrs')
e3 = main.cel2ecef(s26.time, s26.cel[3], s26.radec, 'gcrs')
e4 = main.cel2ecef(s26.time, s26.cel[4], s26.radec, 'gcrs')
e5 = main.cel2ecef(s26.time, s26.cel[5], s26.radec, 'gcrs')
e6 = main.cel2ecef(s26.time, s26.cel[6], s26.radec, 'gcrs')
e7 = main.cel2ecef(s26.time, s26.cel[7], s26.radec, 'gcrs')
e8 = main.cel2ecef(s26.time, s26.cel[8], s26.radec, 'gcrs')
e9 = main.cel2ecef(s26.time, s26.cel[9], s26.radec, 'gcrs')
e10 = main.cel2ecef(s26.time, s26.cel[10], s26.radec, 'gcrs')
e11 = main.cel2ecef(s26.time, s26.cel[11], s26.radec, 'gcrs')
e12 = main.cel2ecef(s26.time, s26.cel[12], s26.radec, 'gcrs')
e13 = main.cel2ecef(s26.time, s26.cel[13], s26.radec, 'gcrs')
e14 = main.cel2ecef(s26.time, s26.cel[14], s26.radec, 'gcrs')
e15 = main.cel2ecef(s26.time, s26.cel[15], s26.radec, 'gcrs')
ev = main.cel2ecef(s26.time, s26.cel[13], s26.radec, 'radec2car')
...
i = np.array([-0.98255908, -0.17588729, -0.06034369])
i1 = np.array([-0.97347843, -0.21852711, -0.067717  ])
i2 = np.array([-0.93587634, -0.33415211, -0.1117044 ])
i3 = np.array([-0.95186305, -0.29777499, -0.07271037])
i4 = np.array([-0.94113921, -0.33612774, -0.03571016])
i5 = np.array([-0.96845555, -0.2480065 ,  0.02422117])
i6 = np.array([-0.97069734, -0.2367948 ,  0.04092638])
i7 = np.array([-0.93947045, -0.34234741, -0.01390949])
i8 = np.array([-0.9380662 , -0.34643665, -0.00367043])
i9 = np.array([-0.94491244, -0.32708648,  0.01244366])
i10 = np.array([-0.96188811, -0.26907518,  0.04868129])
i11 = np.array([-0.91161962, -0.41043325, -0.02223094])
i12 = np.array([-0.96747338, -0.23135386,  0.10232624])
i13 = np.array([-0.96373801, -0.2470969 ,  0.10075779])
i14 = np.array([-0.8985564 , -0.43885493,  0.00165926])
i15 = np.array([-0.90495568, -0.42470873,  0.02603272])

v = np.array([-0.96973706, -0.24364252,  0.01575941]) #boresight vector of the camera
...
#i_v_br = np.dot(main.Rz(np.radians(-s26.imu_tilt[2])), i_v_br1)
