...
'''
File returns position estimate for image belonging to s8 class
'''
...
from ost_output_papakolea import s8
...
phi = s8.imu_tilt[0] #roll
theta = s8.imu_tilt[1] #pitch
psi = s8.imu_tilt[2] #yaw

#converts star coordinates in GCRS (which is a ECI frame) to ITRS (which is a ECEF frame)
e = main.cel2ecef(s8.time, s8.cel[0], s8.radec, 'gcrs')
e1 = main.cel2ecef(s8.time, s8.cel[1], s8.radec, 'gcrs')
e2 = main.cel2ecef(s8.time, s8.cel[2], s8.radec, 'gcrs')
e3 = main.cel2ecef(s8.time, s8.cel[3], s8.radec, 'gcrs')
e4 = main.cel2ecef(s8.time, s8.cel[4], s8.radec, 'gcrs')
e5 = main.cel2ecef(s8.time, s8.cel[5], s8.radec, 'gcrs')
e6 = main.cel2ecef(s8.time, s8.cel[6], s8.radec, 'gcrs')
e7 = main.cel2ecef(s8.time, s8.cel[7], s8.radec, 'gcrs')
e8 = main.cel2ecef(s8.time, s8.cel[8], s8.radec, 'gcrs')
e9 = main.cel2ecef(s8.time, s8.cel[9], s8.radec, 'gcrs')
e10 = main.cel2ecef(s8.time, s8.cel[10], s8.radec, 'gcrs')
e11 = main.cel2ecef(s8.time, s8.cel[11], s8.radec, 'gcrs')
e12 = main.cel2ecef(s8.time, s8.cel[12], s8.radec, 'gcrs')
e13 = main.cel2ecef(s8.time, s8.cel[13], s8.radec, 'gcrs')
ev = main.cel2ecef(s8.time, s8.cel[13], s8.radec, 'radec2car')
...
i = np.array([-0.9102974 , -0.35611616,  0.21104486])
i1 = np.array([-0.89999985, -0.37099477,  0.22882993])
i2 = np.array([-0.86648784, -0.45131063,  0.21334822])
i3 = np.array([-0.90597338, -0.29875453,  0.29992991])
i4 = np.array([-0.91750836, -0.20571188,  0.34038354])
i5 = np.array([-0.89866005, -0.32105142,  0.29889135])
i6 = np.array([-0.90399934, -0.26877091,  0.33248669])
i7 = np.array([-0.88128471, -0.32996601,  0.33831881])
i8 = np.array([-0.86074154, -0.38308908,  0.33521146])
i9 = np.array([-0.88584916, -0.23071192,  0.40254598])
i10 = np.array([-0.87191413, -0.29399408,  0.39157784])
i11 = np.array([-0.84760407, -0.37880741,  0.37158083])
i12 = np.array([-0.85529568, -0.28239811,  0.43441988])
i13 = np.array([-0.85896671, -0.21449163,  0.46494045])


v = np.array([-0.88656693, -0.33998622,  0.3137012 ]) #boresight vector of the camera
...
#i_v_br = np.dot(main.Rz(np.radians(-s8.imu_tilt[2])), i_v_br1)
